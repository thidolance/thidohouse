...
export default function NotasMes({ mes, ano }: Props) {
  ...
  return (
    <>
      {/* Painel */}
      {aberto && (
        <div className="fixed bottom-20 right-4 z-50 w-80 bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl border border-indigo-100 dark:border-purple-500/30 flex flex-col overflow-hidden animate-in slide-in-from-bottom-4 duration-200">
          {/* Header */}
          <div className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-indigo-600 dark:from-purple-600 to-violet-700 text-white">
          ...
          <div className="flex flex-col flex-1 p-3 gap-2">
            {loading ? (
              <div className="flex-1 flex items-center justify-center py-8">
                <div className="h-5 w-5 rounded-full border-2 border-indigo-400 dark:border-purple-400 border-t-transparent animate-spin" />
              </div>
            ) : (
              <textarea
                ref={textareaRef}
            ...
            <button
              ...
              className="flex items-center justify-center gap-1.5 w-full py-2 rounded-xl text-sm font-medium transition-colors disabled:opacity-40 disabled:cursor-not-allowed bg-indigo-600 dark:bg-purple-600 hover:bg-indigo-700 dark:hover:bg-purple-700 text-white"
            >
