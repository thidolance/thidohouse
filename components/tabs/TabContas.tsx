...
function ScopeDialog({ title, desc, labelFuture, onFuture, onThisOnly, onCancel }: {
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-zinc-900 rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-4 animate-[fadeIn_0.15s_ease]">
        <h3 className="font-bold text-zinc-100 text-base">{title}</h3>
        <p className="text-sm text-zinc-400 leading-relaxed">{desc}</p>
        <div className="space-y-2 pt-1">
          <button onClick={onFuture} className="w-full py-2.5 rounded-xl text-sm font-semibold bg-purple-600 text-white hover:bg-purple-700 transition-colors">
            {labelFuture}
          </button>
          <button onClick={onThisOnly} className="w-full py-2.5 rounded-xl text-sm font-medium bg-zinc-700 text-white hover:bg-zinc-800 transition-colors">
            Só este mês
          </button>
          <button onClick={onCancel} className="w-full py-2.5 rounded-xl text-sm text-zinc-400 border border-zinc-800 hover:bg-zinc-950 transition-colors">
            Cancelar
          </button>
        </div>
      </div>
...
export default function TabContas({ mes, ano }: Props) {
  ...
  function ContaRow({ c, barColor }: { c: Conta; barColor: string }) {
    ...
    return (
      <div className={`group flex items-center gap-3 px-4 py-3.5 transition-all ${pago ? 'bg-emerald-500/10/70' : 'hover:bg-zinc-950/80'}`}>
        ...
        <button
          ...
          className={`w-7 h-7 rounded-full border-2 flex items-center justify-center transition-all flex-shrink-0 shadow-sm ${
            pago ? 'bg-emerald-500 border-emerald-500 text-white' : 'border-zinc-800 hover:border-emerald-400 hover:bg-emerald-500/10'
          }`}
        >
          {pago && <Check />}
        </button>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 flex-wrap">
            <p className={`font-semibold text-sm leading-tight ${pago ? 'line-through text-zinc-500' : 'text-zinc-100'}`}>{c.descricao}</p>
          </div>
          <div className="flex items-center gap-2 mt-0.5 flex-wrap">
            ...
            <span className="text-[11px] text-zinc-500">dia {c.vencimento}</span>
          </div>
          {c.totalParcelas && c.parcelaAtual && (
            ...
            >
              <div className="flex-1 max-w-[100px] bg-zinc-800 rounded-full h-1.5">
                <div
                  className="h-1.5 rounded-full transition-all"
                  style={{ width: `${(c.parcelaAtual / c.totalParcelas) * 100}%`, backgroundColor: '#10b981', opacity: pago ? 0.5 : 0.8 }}
                />
              </div>
              <span className="text-[10px] text-zinc-500 tabular-nums">{c.parcelaAtual}/{c.totalParcelas}</span>
              <span className="text-zinc-600 group-hover/prog:text-purple-400 transition-colors"><ChevronRight /></span>
            </button>
          )}
        </div>
        <div className="flex items-center gap-2 flex-shrink-0">
          <span className={`font-bold text-sm tabular-nums ${pago ? 'text-emerald-400' : 'text-zinc-100'}`}>{fmt(c.valor)}</span>
          <div className="w-[5.5rem] flex items-center gap-0.5 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
            {!c.parcelaAtual && (
              <button onClick={() => handleToggleFixa(c)} title={c.fixa ? 'Remover recorrência' : 'Marcar como fixa'}
                className={`p-1.5 rounded-lg transition-colors ${c.fixa ? 'text-amber-500 bg-amber-500/10' : 'text-zinc-500 hover:text-amber-500 hover:bg-amber-500/10'}`}>
                <PinIcon filled={c.fixa} />
              </button>
            )}
            <button onClick={() => abrirEditar(c)} className="p-1.5 rounded-lg text-zinc-500 hover:text-purple-400 hover:bg-purple-500/10 transition-colors"><Pencil /></button>
            <button onClick={() => iniciarDelete(c)} className="p-1.5 rounded-lg text-zinc-500 hover:text-red-500 hover:bg-red-500/10 transition-colors"><Trash /></button>
          </div>
        </div>
      </div>
    );
  ...
  function SectionHeader({ label, count, total: t }: { label: string; count: number; total: number }) {
    return (
      <div className="px-4 py-2 bg-gradient-to-r from-slate-50 to-transparent flex items-center justify-between border-b border-zinc-800/80">
        <p className="text-[11px] font-bold text-zinc-500 uppercase tracking-widest">{label}</p>
        <p className="text-[11px] text-zinc-500">{count} · {fmt(t)}</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between gap-2">
        <div>
          <h2 className="text-lg font-bold text-zinc-100">Contas do Mês</h2>
          <p className="text-xs text-zinc-500">{countPago}/{countTotal} pagas · {fmt(totalPendente)} pendentes</p>
        </div>
      ...
      {pendentesHoje.length > 0 && (
        <div className="flex items-start gap-3 bg-amber-500/10 border border-amber-200 rounded-xl p-3.5">
          <span className="text-lg leading-none mt-0.5">⚠️</span>
          <div>
            <p className="text-amber-300 text-sm font-semibold">{pendentesHoje.length} conta(s) vencida(s) ou vencendo hoje</p>
            <p className="text-amber-400 text-xs mt-0.5">{pendentesHoje.map((c) => c.descricao).join(' · ')}</p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <div className="bg-gradient-to-br from-emerald-500 via-emerald-500 to-teal-400 rounded-2xl px-4 py-3 text-white shadow-lg shadow-emerald-500/20/60">
        ...
        <div className="bg-gradient-to-br from-amber-500 via-orange-500 to-orange-400 rounded-2xl px-4 py-3 text-white shadow-lg shadow-amber-500/20/60">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card>
          ...
          {pieValues.length > 0 ? (
            ...
            <div className="h-[240px] flex flex-col items-center justify-center text-zinc-500 gap-2">
        ...
        <Card>
          <p className="font-semibold text-zinc-200 text-sm mb-3">Por Tipo de Conta</p>
          {tipoValues.length > 0 ? (
            <div style={{ height: 240 }}>
      ...
      <Card className="!p-0 overflow-hidden">
          ...
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              <input value={busca} onChange={(e) => setBusca(e.target.value)} placeholder="Buscar por descrição..."
                className="w-full pl-9 pr-3 py-2 text-sm border border-zinc-800 rounded-xl bg-zinc-900 text-zinc-50 focus:outline-none focus:ring-2 focus:ring-purple-300 placeholder:text-zinc-500" />
            </div>
            <div className="relative">
              <div className="absolute left-2.5 top-1/2 -translate-y-1/2 pointer-events-none text-zinc-500"><SortIcon /></div>
              <select value={ordenacao} onChange={(e) => setOrdenacao(e.target.value as OrdenacaoKey)}
                className="pl-8 pr-7 py-2 text-sm border border-zinc-800 rounded-xl bg-zinc-900 text-zinc-200 focus:outline-none focus:ring-2 focus:ring-purple-300 appearance-none cursor-pointer">
          ...
          <div className="flex flex-wrap gap-1.5">
            {(['todos', 'pendente', 'pago'] as const).map((s) => (
              <button key={s} onClick={() => setFiltroStatus(s)}
                className={`px-3 py-1 rounded-full text-xs font-semibold transition-all border ${
                  filtroStatus === s
                    ? s === 'todos' ? 'bg-zinc-800 text-white border-zinc-700'
                      : s === 'pago' ? 'bg-emerald-500 text-white border-emerald-500'
                      : 'bg-amber-400 text-white border-amber-400'
                    : 'bg-zinc-900 text-zinc-400 border-zinc-800 hover:border-zinc-700 hover:bg-zinc-950'
                }`}
              >
            ...
            {([['todos','Tudo'],['contas','Contas'],['cartoes','Cartões'],['empresa','Empresa']] as const).map(([v, l]) => (
              <button key={v} onClick={() => { setFiltroTipo(v); setFiltroCategoria(''); }}
                className={`px-3 py-1 rounded-full text-xs font-semibold transition-all border ${
                  filtroTipo === v ? 'bg-purple-600 text-white border-purple-600' : 'bg-zinc-900 text-zinc-400 border-zinc-800 hover:border-zinc-700'
                }`}
              >{l}</button>
            ))}
            {(filtroTipo === 'todos' || filtroTipo === 'contas') && cats.length > 0 && (
              <>
                ...
                <select value={filtroCategoria} onChange={(e) => setFiltroCategoria(e.target.value)}
                  className={`px-3 py-1 rounded-full text-xs font-semibold border transition-all bg-zinc-900 cursor-pointer focus:outline-none ${filtroCategoria ? 'border-purple-400 text-purple-400' : 'border-zinc-800 text-zinc-400'}`}>
            ...
            {filtrosAtivos && (
              <button onClick={limparFiltros} className="ml-auto px-3 py-1 rounded-full text-xs font-semibold text-zinc-500 border border-zinc-800 hover:bg-zinc-950 transition-colors">
                ✕ Limpar
              </button>
            )}
          </div>
        ...
        {filtrosAtivos && totalVisiveis >= 0 && (
          <div className="px-4 py-1.5 bg-purple-500/10/60 border-b border-purple-500/30/50">
            <p className="text-xs text-purple-400 font-medium">{totalVisiveis} resultado(s){filtroCategoria && <span> — categoria <strong>{filtroCategoria}</strong></span>}</p>
          </div>
        )}
        ...
        {loading ? (
          ...
          <div className="py-16 text-center">
            ...
            {filtrosAtivos && (
              <button onClick={limparFiltros} className="mt-2 text-xs text-purple-400 hover:underline">Limpar filtros</button>
            )}
          </div>
        ) : (
          <div className="divide-y divide-zinc-800/80">
            ...
            {mostrarCartoes && cartoesFiltrados.length > 0 && (
              <div>
                ...
                {cartoesFiltrados.map((c) => (
                  <div key={c.cartaoId} className={`group flex items-center gap-3 px-4 py-3.5 transition-all ${c.status === 'pago' ? 'bg-emerald-500/10/70' : 'hover:bg-zinc-950/80'}`}>
                    ...
                    <button onClick={() => handleToggleCartao(c.cartaoId, c.status)}
                      className={`w-7 h-7 rounded-full border-2 flex items-center justify-center transition-all flex-shrink-0 shadow-sm ${c.status === 'pago' ? 'bg-emerald-500 border-emerald-500 text-white' : 'border-zinc-800 hover:border-emerald-400 hover:bg-emerald-500/10'}`}>
                      {c.status === 'pago' && <Check />}
                    </button>
                    <div className="flex-1 min-w-0">
                      <p className={`font-semibold text-sm ${c.status === 'pago' ? 'line-through text-zinc-500' : 'text-zinc-100'}`}>Fatura {c.cartaoNome}</p>
                      <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                        ...
                        {c.valor !== c.valorCalculado && (
                          <span className="text-[11px] text-amber-400" title={`Soma das compras: ${fmt(c.valorCalculado)}`}>valor ajustado</span>
                        )}
                      </div>
                    </div>
                    <div className="flex items-center gap-2 flex-shrink-0">
                      <span className={`font-bold text-sm tabular-nums ${c.status === 'pago' ? 'text-emerald-400' : 'text-zinc-100'}`}>{fmt(c.valor)}</span>
                      <div className="w-[5.5rem] flex items-center justify-end gap-0.5 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                        <button onClick={() => abrirEditFatura(c)} title="Editar valor da fatura" className="p-1.5 rounded-lg text-zinc-500 hover:text-purple-400 hover:bg-purple-500/10 transition-colors"><Pencil /></button>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            {mostrarEmpresa && empresaSum > 0 && !(filtroStatus !== 'todos' && filtroStatus !== empresaStatus) && (
              <div>
                ...
                <div className={`group flex items-center gap-3 px-4 py-3.5 transition-all ${empresaStatus === 'pago' ? 'bg-emerald-500/10/70' : 'hover:bg-zinc-950/80'}`}>
                  <div className="w-0.5 h-10 rounded-full flex-shrink-0 bg-sky-400" />
                  <button onClick={handleToggleEmpresaAll}
                    className={`w-7 h-7 rounded-full border-2 flex items-center justify-center transition-all flex-shrink-0 shadow-sm ${empresaStatus === 'pago' ? 'bg-emerald-500 border-emerald-500 text-white' : 'border-zinc-800 hover:border-emerald-400 hover:bg-emerald-500/10'}`}>
                    {empresaStatus === 'pago' && <Check />}
                  </button>
                  <div className="flex-1 min-w-0">
                    <p className={`font-semibold text-sm ${empresaStatus === 'pago' ? 'line-through text-zinc-500' : 'text-zinc-100'}`}>Empresa</p>
                    <div className="flex items-center gap-1.5 mt-0.5">
                      ...
                      <span className="text-[11px] text-zinc-500">custos do mês</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <span className={`font-bold text-sm tabular-nums ${empresaStatus === 'pago' ? 'text-emerald-400' : 'text-zinc-100'}`}>{fmt(empresaSum)}</span>
      ...
      {showModal && (
        <Modal title={editId ? 'Editar Conta' : 'Nova Conta'} onClose={fecharModal}>
          <form onSubmit={handleSave} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-zinc-200 mb-1">Categoria</label>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="block text-sm font-medium text-zinc-200 mb-1">Vencimento (dia)</label>
            ...
            <div>
              <label className="block text-sm font-medium text-zinc-200 mb-1">Parcelas <span className="text-zinc-500 font-normal">(opcional)</span></label>
              <div className="grid grid-cols-2 gap-3">
            ...
            {!isParcelada && (
              <label className="flex items-center gap-3 cursor-pointer select-none p-3 rounded-xl border border-zinc-800 hover:bg-zinc-950 transition-colors">
                <div onClick={() => setForm({ ...form, fixa: !form.fixa })}
                  className={`w-11 h-6 rounded-full transition-colors flex items-center flex-shrink-0 ${form.fixa ? 'bg-amber-400' : 'bg-zinc-800'}`}>
                  <span className={`w-5 h-5 bg-white rounded-full shadow transition-transform mx-0.5 ${form.fixa ? 'translate-x-5' : 'translate-x-0'}`} />
                </div>
                <div>
                  <p className="text-sm font-medium text-zinc-200">Conta Fixa</p>
                  <p className="text-xs text-zinc-500">Aparece automaticamente todo mês</p>
                </div>
              </label>
            )}
      ...
      {editFatura && (
        <Modal title={`Fatura ${editFatura.cartaoNome}`} onClose={() => setEditFatura(null)}>
          <form onSubmit={handleSaveFaturaValor} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-zinc-200 mb-1">Valor da fatura (R$)</label>
              <input
                autoFocus
                ...
                inputMode="decimal"
              />
              <p className="text-xs text-zinc-500 mt-1">Soma das compras: {fmt(editFatura.valorCalculado)}</p>
            </div>
            <div className="flex gap-3 pt-1">
              {editFatura.valor !== editFatura.valorCalculado && (
                <button type="button" onClick={handleResetFaturaValor} className="flex-1 py-2.5 border border-zinc-800 rounded-xl text-sm text-zinc-300 hover:bg-zinc-950">Usar soma</button>
              )}
              <button type="submit" className="flex-1 py-2.5 bg-purple-600 text-white rounded-xl text-sm font-semibold hover:bg-purple-700 shadow-sm">Salvar</button>
            </div>
          </form>
          ...
          <div className="space-y-3">
            ...
            {cats.map((cat) => (
              <div key={cat.id} className="flex items-center gap-3 p-3 rounded-xl border border-zinc-800 hover:bg-zinc-950">
                ...
                <button onClick={() => handleDeleteCat(cat.id!)} className="p-1.5 rounded-lg hover:bg-red-500/10 text-zinc-600 hover:text-red-400 transition-colors"><Trash /></button>
              </div>
            ))}
            {!showCatForm ? (
              <button onClick={() => setShowCatForm(true)}
                className="w-full py-2.5 border-2 border-dashed border-zinc-800 rounded-xl text-sm text-zinc-500 hover:border-purple-300 hover:text-purple-400 flex items-center justify-center gap-2">
                <Plus /> Nova Categoria
              </button>
            ) : (
              <form onSubmit={handleSaveCat} className="border border-zinc-800 rounded-xl p-4 space-y-3 bg-zinc-950">
                ...
                <div className="flex gap-2">
                  <button type="button" onClick={() => { setShowCatForm(false); setFormCat({ nome: '', cor: '#6366f1' }); }} className="flex-1 py-2 border border-zinc-800 rounded-xl text-xs text-zinc-300 hover:bg-zinc-800">Cancelar</button>
                  <button type="submit" className="flex-1 py-2 bg-purple-600 text-white rounded-xl text-xs font-semibold hover:bg-purple-700">Salvar</button>
                </div>
              </form>
            )}
          </div>
      ...
      {acompanhar && (
        <Modal title="Acompanhamento da Conta" onClose={() => setAcompanhar(null)}>
          {loadingAcompanhar || !acompanhamentoStats ? (
            ...
            <div className="space-y-5">
              <div>
                <p className="font-bold text-zinc-100 text-base leading-tight">{acompanhar.descricao}</p>
                <span
              ...
              <div className="flex items-center gap-5">
                <div className="relative flex-shrink-0">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <span className="text-lg font-bold text-zinc-100 tabular-nums">{acompanhamentoStats.countPago}</span>
                    <span className="text-[10px] text-zinc-500">de {acompanhamentoStats.totalParcelas}</span>
                  </div>
                </div>
                <div className="flex-1 space-y-1">
                  <p className="text-[11px] text-zinc-500 uppercase tracking-wide font-medium">Valor da parcela</p>
                  <p className="text-2xl font-bold text-zinc-100 tabular-nums">{fmt(acompanhamentoStats.valorParcela)}</p>
                  <p className="text-xs text-zinc-500">{acompanhamentoStats.percent.toFixed(0)}% concluído</p>
                </div>
              </div>

              {/* Já pago vs restante */}
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-emerald-500/10 border border-emerald-100 rounded-2xl p-3.5">
                  <p className="text-emerald-400 text-[11px] font-semibold uppercase tracking-wide">Já pago</p>
                  <p className="text-lg font-bold text-emerald-300 mt-1 tabular-nums">{fmt(acompanhamentoStats.valorPago)}</p>
                  <p className="text-[11px] text-emerald-500 mt-0.5">{acompanhamentoStats.countPago} parcela(s)</p>
                </div>
                <div className="bg-purple-500/10 border border-purple-500/30 rounded-2xl p-3.5">
                  <p className="text-purple-400 text-[11px] font-semibold uppercase tracking-wide">Para quitar</p>
                  <p className="text-lg font-bold text-purple-700 mt-1 tabular-nums">{fmt(acompanhamentoStats.valorRestante)}</p>
                  <p className="text-[11px] text-purple-400 mt-0.5">{acompanhamentoStats.countRestante} parcela(s)</p>
                </div>
              </div>

              {/* Linha do tempo das parcelas */}
              <div>
                <p className="text-[11px] text-zinc-500 uppercase tracking-wide font-medium mb-2">Linha do tempo</p>
                <div className="flex flex-wrap gap-1.5">
                  {Array.from({ length: acompanhamentoStats.totalParcelas }, (_, i) => {
                    ...
                    return (
                      <div
                        ...
                        className={`h-6 flex-1 min-w-[16px] rounded-md transition-colors ${paga ? 'bg-emerald-400' : 'bg-zinc-800'} ${atual ? 'ring-2 ring-purple-500 ring-offset-1' : ''}`}
                      />
                    );
                  })}
                </div>
