...
function CircularProgress({ percent, size = 88, stroke = 8 }: { percent: number; size?: number; stroke?: number }) {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="-rotate-90">
      <circle cx={size / 2} cy={size / 2} r={r} fill="none" stroke="rgba(113,113,122,0.2)" strokeWidth={stroke} />
      <circle
...
function ScopeDialog({ title, desc, labelFuture, onFuture, onThisOnly, onCancel }: {
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-4 animate-[fadeIn_0.15s_ease]">
        <h3 className="font-bold text-slate-800 dark:text-zinc-100 text-base">{title}</h3>
        <p className="text-sm text-slate-500 dark:text-zinc-400 leading-relaxed">{desc}</p>
        <div className="space-y-2 pt-1">
          <button onClick={onFuture} className="w-full py-2.5 rounded-xl text-sm font-semibold bg-indigo-600 dark:bg-purple-600 text-white hover:bg-indigo-700 dark:hover:bg-purple-700 transition-colors">
            {labelFuture}
          </button>
          <button onClick={onThisOnly} className="w-full py-2.5 rounded-xl text-sm font-medium bg-slate-700 dark:bg-zinc-700 text-white hover:bg-slate-800 dark:hover:bg-zinc-700 transition-colors">
            Só este mês
          </button>
          <button onClick={onCancel} className="w-full py-2.5 rounded-xl text-sm text-slate-500 dark:text-zinc-400 border border-slate-200 dark:border-zinc-800 hover:bg-slate-50 dark:hover:bg-zinc-800 transition-colors">
            Cancelar
          </button>
        </div>
      </div>
    ...
    return (
      <div className={`group flex items-center gap-3 px-4 py-3.5 transition-all ${pago ? 'border-l-2 border-emerald-400 dark:border-emerald-500 bg-gradient-to-r from-emerald-100 via-emerald-50 to-transparent dark:from-emerald-500/20 dark:via-emerald-500/5 dark:to-transparent' : 'hover:bg-slate-50/80 dark:hover:bg-zinc-800/80'}`}>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <span className={`font-bold text-sm tabular-nums ${pago ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-800 dark:text-zinc-100'}`}>{fmt(c.valor)}</span>
  ...
  return (
    <div className="space-y-5">
      ...
      {pendentesHoje.length > 0 && (
        <div className="flex items-start gap-3 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 rounded-xl p-3.5">
          <span className="text-lg leading-none mt-0.5">⚠️</span>
          <div>
            <p className="text-amber-800 dark:text-amber-300 text-sm font-semibold">{pendentesHoje.length} conta(s) vencida(s) ou vencendo hoje</p>
            <p className="text-amber-600 dark:text-amber-400 text-xs mt-0.5">{pendentesHoje.map((c) => c.descricao).join(' · ')}</p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <Card>
          <p className="font-semibold text-slate-700 dark:text-zinc-200 text-sm mb-3">Por Tipo de Conta</p>
          {tipoValues.length > 0 ? (
            <div style={{ height: 240 }}>
      ...
      <Card className="!p-0 overflow-hidden">
          ...
          <div className="flex flex-wrap gap-1.5">
            {(['todos', 'pendente', 'pago'] as const).map((s) => (
              <button key={s} onClick={() => setFiltroStatus(s)}
                className={`px-3 py-1 rounded-full text-xs font-semibold transition-all border ${
                  filtroStatus === s
                    ? s === 'todos' ? 'bg-slate-800 dark:bg-zinc-800 text-white border-slate-800'
                      : s === 'pago' ? 'bg-emerald-500 text-white border-emerald-500'
                      : 'bg-amber-400 text-white border-amber-400'
                    : 'bg-white dark:bg-zinc-900 text-slate-500 dark:text-zinc-400 border-slate-200 dark:border-zinc-800 hover:border-slate-300 dark:hover:border-zinc-600 hover:bg-slate-50 dark:hover:bg-zinc-800'
                }`}
              >
            ...
            {([['todos','Tudo'],['contas','Contas'],['cartoes','Cartões'],['empresa','Empresa']] as const).map(([v, l]) => (
              <button key={v} onClick={() => { setFiltroTipo(v); setFiltroCategoria(''); }}
                className={`px-3 py-1 rounded-full text-xs font-semibold transition-all border ${
                  filtroTipo === v ? 'bg-indigo-600 dark:bg-purple-600 text-white border-indigo-600 dark:border-purple-600' : 'bg-white dark:bg-zinc-900 text-slate-500 dark:text-zinc-400 border-slate-200 dark:border-zinc-800 hover:border-slate-300 dark:hover:border-zinc-600'
                }`}
              >{l}</button>
            ))}
            {(filtroTipo === 'todos' || filtroTipo === 'contas') && cats.length > 0 && (
              <>
                ...
                <select value={filtroCategoria} onChange={(e) => setFiltroCategoria(e.target.value)}
                  className={`px-3 py-1 rounded-full text-xs font-semibold border transition-all bg-white dark:bg-zinc-900 cursor-pointer focus:outline-none ${filtroCategoria ? 'border-indigo-400 dark:border-purple-400 text-indigo-600 dark:text-purple-400' : 'border-slate-200 dark:border-zinc-800 text-slate-500 dark:text-zinc-400'}`}>
        ...
        {filtrosAtivos && totalVisiveis >= 0 && (
          <div className="px-4 py-1.5 bg-indigo-50/60 dark:bg-purple-500/10 border-b border-indigo-100/50 dark:border-purple-500/30">
            <p className="text-xs text-indigo-500 dark:text-purple-400 font-medium">{totalVisiveis} resultado(s){filtroCategoria && <span> — categoria <strong>{filtroCategoria}</strong></span>}</p>
          </div>
        )}
        ...
        {loading ? (
          ...
          <div className="py-16 text-center">
            ...
            {filtrosAtivos && (
              <button onClick={limparFiltros} className="mt-2 text-xs text-indigo-500 dark:text-purple-400 hover:underline">Limpar filtros</button>
            )}
          </div>
        ) : (
          <div className="divide-y divide-slate-50/80 dark:divide-zinc-800/80">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                        ...
                        {c.valor !== c.valorCalculado && (
                          <span className="text-[11px] text-amber-600 dark:text-amber-400" title={`Soma das compras: ${fmt(c.valorCalculado)}`}>valor ajustado</span>
                        )}
                      </div>
                    </div>
                    <div className="flex items-center gap-2 flex-shrink-0">
            ...
            {mostrarEmpresa && empresaSum > 0 && !(filtroStatus !== 'todos' && filtroStatus !== empresaStatus) && (
              <div>
                ...
                <div className={`group flex items-center gap-3 px-4 py-3.5 transition-all ${empresaStatus === 'pago' ? 'border-l-2 border-emerald-400 dark:border-emerald-500 bg-gradient-to-r from-emerald-100 via-emerald-50 to-transparent dark:from-emerald-500/20 dark:via-emerald-500/5 dark:to-transparent' : 'hover:bg-slate-50/80 dark:hover:bg-zinc-800/80'}`}>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <span className={`font-bold text-sm tabular-nums ${empresaStatus === 'pago' ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-800 dark:text-zinc-100'}`}>{fmt(empresaSum)}</span>
        ...
        <Modal title={editId ? 'Editar Conta' : 'Nova Conta'} onClose={fecharModal}>
          <form onSubmit={handleSave} className="space-y-4">
            ...
            {!isParcelada && (
              <label className="flex items-center gap-3 cursor-pointer select-none p-3 rounded-xl border border-slate-100 dark:border-zinc-800 hover:bg-slate-50 dark:hover:bg-zinc-800 transition-colors">
                <div onClick={() => setForm({ ...form, fixa: !form.fixa })}
                  className={`w-11 h-6 rounded-full transition-colors flex items-center flex-shrink-0 ${form.fixa ? 'bg-amber-400' : 'bg-slate-200 dark:bg-zinc-800'}`}>
            ...
            <div className="flex gap-3 pt-1">
              <button type="button" onClick={fecharModal} className="flex-1 py-2.5 border border-slate-200 dark:border-zinc-800 rounded-xl text-sm text-slate-600 dark:text-zinc-300 hover:bg-slate-50 dark:hover:bg-zinc-800">Cancelar</button>
      ...
      {editFatura && (
        <Modal title={`Fatura ${editFatura.cartaoNome}`} onClose={() => setEditFatura(null)}>
          <form onSubmit={handleSaveFaturaValor} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-zinc-200 mb-1">Valor da fatura (R$)</label>
              <input
                autoFocus
            ...
            <div className="flex gap-3 pt-1">
              {editFatura.valor !== editFatura.valorCalculado && (
                <button type="button" onClick={handleResetFaturaValor} className="flex-1 py-2.5 border border-slate-200 dark:border-zinc-800 rounded-xl text-sm text-slate-600 dark:text-zinc-300 hover:bg-slate-50 dark:hover:bg-zinc-800">Usar soma</button>
              )}
              <button type="submit" className="flex-1 py-2.5 bg-indigo-600 dark:bg-purple-600 text-white rounded-xl text-sm font-semibold hover:bg-indigo-700 dark:hover:bg-purple-700 shadow-sm">Salvar</button>
            </div>
          </form>
        ...
        <Modal title="Configurações — Contas" onClose={() => { setShowConfig(false); setShowCatForm(false); }}>
          <div className="space-y-3">
            ...
            {!showCatForm ? (
              ...
              <form onSubmit={handleSaveCat} className="border border-slate-200 dark:border-zinc-800 rounded-xl p-4 space-y-3 bg-slate-50 dark:bg-zinc-950">
                ...
                <div className="flex gap-2">
                  <button type="button" onClick={() => { setShowCatForm(false); setFormCat({ nome: '', cor: '#6366f1' }); }} className="flex-1 py-2 border border-slate-200 dark:border-zinc-800 rounded-xl text-xs text-slate-600 dark:text-zinc-300 hover:bg-white dark:hover:bg-zinc-800">Cancelar</button>
